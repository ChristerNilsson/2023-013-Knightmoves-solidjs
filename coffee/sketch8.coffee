...
import {abs,N,col,row,log,Position,range,r4r,sum} from '../js/utils8.js' # signal effect
import {svg,rect,text,circle,g} from '../js/utils8.js'
...
makeKnightHops = (knight) =>
	...
	for dc in [-2,-1,1,2]
		for dr in [-2,-1,1,2]
			...
			r2 = r + dr
			index = c2+8*r2
			if c2 in range(8) and r2 in range(8) and index in targets then res.push index  # targets()
	res.sort (a,b) -> a-b
	show 'knightHops', res
